...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, pattern=None):
        ...
        self.adapter_alpha = nn.Parameter(torch.Tensor([0.1+np.random.normal(0.0, 0.05, 1)[0]]))
        self.adapter_beta = nn.Parameter(torch.Tensor([0.1+np.random.normal(0.0, 0.05, 1)[0]]))
    ...
    def attention(self, x: torch.Tensor, use_mask=None, prob=0.0):
        ...
        def reshape_heads(t):
            t = t.permute(1, 0, 2) 
            t = t.reshape(B, L, num_heads, head_dim).transpose(1, 2)  
            return t.reshape(B * num_heads, L, head_dim)  
        ...
        if use_mask is not None:    
            # use_cls = True
            mask = use_mask.view(B, -1)
            patch_attn = attn_logits[:, :, :]
            cls_mask = torch.ones((B, 1)).to(mask.device)
            mask = torch.cat([cls_mask, mask], dim=-1)

            randm0 = torch.rand(mask.shape).to(mask.device)
            mask = torch.where((mask == 0) & (randm0 < prob), torch.tensor(1), mask)

            mask_i = mask.unsqueeze(2).bool()  
            mask_j = mask.unsqueeze(1).bool()  

            attn_mask = (mask_i & mask_j).int()  
            max_value = patch_attn.max(-1)[0].unsqueeze(-1)   
            min_value = patch_attn.min(-1)[0].unsqueeze(-1)

            attn_mask = attn_mask.unsqueeze(1).repeat(1, num_heads, 1, 1).view(B * num_heads, (L), (L))  
            patch_attn += (attn_mask>0).float()  * self.adapter_alpha * (max_value - patch_attn) 
            patch_attn -= (~(attn_mask>0)).float()  * self.adapter_beta * (patch_attn - min_value) 

        attn_logits = attn_logits * scale
        if self.attn_mask is not None:
            attn_logits += self.attn_mask 

        attn_weights = F.softmax(attn_logits, dim=-1)
        attn_weights = F.dropout(attn_weights, p=self.attn.dropout, training=self.training)
        attn_output = torch.bmm(attn_weights, v)

        attn_output = attn_output.view(B, num_heads, L, head_dim).transpose(1, 2).reshape(B, L, D)
    ...
    def forward_ori(self, x: torch.Tensor, use_mask=None, prob=0.0):
        x = x + self.attention(self.ln_1(x), use_mask=use_mask, prob=prob)
    ...
    def forward(self, x: torch.Tensor, modality=None, index=None, last_prompt=None, prompt_sign=True,
                adapter_sign=True, use_mask=None, prob=0.0):
        if prompt_sign and adapter_sign:
            ...
            return self.forward_ori(x, use_mask=use_mask, prob=prob)
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, modality=None, index=None, last_prompt=None, use_mask=None, prob=0.0):
        return self.resblocks(x, modality, index, last_prompt, use_mask=use_mask, prob=prob)
...
class VisionTransformer(nn.Module):
    ...
    def forward_old(self, x: torch.Tensor, cv_emb=None, modality=None, text_inverse=None, use_mask=None, prob=0.0):
        ...
        for i in range(len(self.transformer.resblocks)):
            if self.prompt_sign and self.adapter_sign:
                ...
                x = self.transformer.resblocks[i](x, modality, i, None, prompt_sign=False, adapter_sign=False, use_mask=use_mask, prob=prob)
    ...
    def forward(self, x: torch.Tensor, cv_emb=None, modality=None, text_inverse=None, use_mask=None, prob=0.0):
        if self.forward_type == 'new':
            return self.forward_new(x, cv_emb, modality, text_inverse, use_mask=use_mask, prob=prob)
        else:
            return self.forward_old(x, cv_emb, modality, text_inverse, use_mask=use_mask, prob=prob)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, cv_embed, modality, text_inverse=None,use_mask=None, prob=0.0):
        return self.visual(image.type(self.dtype), cv_embed, modality, text_inverse, use_mask=use_mask, prob=prob)
